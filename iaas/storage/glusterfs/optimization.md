...
# 目录预读的优化
gluster volume set dht-vol performance.readdir-ahead on
...
gluster volume set dht-vol   performance.write-behind-window-size 64mb
...
```bash
gluster volume profile    one  start
gluster volume profile    one  info
```